...
class SsPj(models.Model):
    ...
    _sql_constraints = [('unique_pj', 'unique(pj_id, account_pj_date)', 'pj_id should be unique per pj!')]
...
class SsPjDetail(models.Model):
    ...
    # 小計項目を追加する
    subtotal = fields.Char(u'小計', compute='_compute_subtotal')

    carfare = fields.Char(u'交通費')
    amount_subtotal = fields.Char(u'売上合計')
